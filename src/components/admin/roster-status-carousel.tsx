...
export function RosterStatusCarousel({ people }: { people: RosterPreviewPerson[] }) {
  ...
  function selectGroup(key: (typeof groups)[number]["key"]) {
    setSelected(key);
    setOpen(true);
  }
  ...
  return (
    ...
    >
      ...
      <div
        className={`roster-carousel mt-5 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 ${open ? "roster-carousel-expanded" : ""}`}
        role="tablist"
      ...
      >
        {groups.map((group) => {
          ...
          return (
            <div key={group.key} className="roster-card-slot min-w-[12.5rem] snap-start">
              <button
                type="button"
                role="tab"
                aria-selected={isSelected}
                aria-expanded={isSelected && open}
                className={`roster-summary-card w-full text-left ${isSelected ? "roster-summary-card-selected" : ""} ${open && !isSelected ? "roster-summary-card-muted" : ""}`}
                onClick={() => selectGroup(group.key)}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") selectGroup(group.key);
                }}
              >
      ...
      {open ? (
        <div
          className="roster-expanded-surface mt-3"
          role="region"
          aria-label={`${current.label} roster`}
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="admin-eyebrow">Selected group</p>
              <h3 className="mt-1 text-2xl font-semibold">{current.label}</h3>
          ...
          <div className="roster-expanded-body mt-5 overflow-auto">
            {filtered.length ? (
