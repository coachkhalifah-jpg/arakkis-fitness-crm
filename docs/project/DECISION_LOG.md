...
| 2026-09-22 / DEC-058 / `f52da3f` | Accept tip `f52da3f` as the product candidate SHA. Keep confirmation tokens at 24h. Remember-this-device is how returning participants see and manage all bookings; strongly encourage it on confirmation. Events marks already-booked classes with manage links. Disconnected `/manage-bookings` and expired confirmation use calm recovery (never 404-feel) with Browse events / saved link. Next work is narrow engineering + QA on that candidate lineage only. | Product candidate freeze; supersedes DEC-055 presentation conflicts. |
