...
- BR-107: Only one active confirmation token exists per Registration Group. Regeneration revokes the previous token. Valid tokens are read-only and reusable only for the same confirmation page. Expired or revoked tokens must present a clear expired/unavailable confirmation state (HTTP 200 page; ICS uses 410 when expired) and must never hard-404 the confirmation page. When a remembered-device cookie is present, offer **View your bookings**. Malformed or unknown tokens remain a generic invalid-link response without disclosing other participants.
