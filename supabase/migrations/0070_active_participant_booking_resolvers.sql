-- INT-002: remembered-device booking access requires an ACTIVE Participant.
-- Historical migrations remain immutable; confirmation-scoped access is unchanged.
...
begin
  ...
  select d.participant_id into v_participant_id
    ...
    join public.participants p on p.id = d.participant_id and p.status = 'ACTIVE'
   where d.token_hash = digest(p_token, 'sha256') and d.revoked_at is null and d.expires_at > now();
...
revoke all on function public.phase10_resolve_booking_participant(text) from public;
