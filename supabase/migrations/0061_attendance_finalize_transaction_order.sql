-- The attendance invariant trigger intentionally requires NO_SHOW rows to be
-- written only after the Event enters FINALIZED.  Keep that integrity guard
-- and order the single transaction accordingly.
...
begin
  ...
  -- This update must precede NO_SHOW inserts/updates because the authoritative
  -- attendance_invariants trigger validates the Event state before each row.
