-- Phase 4C: confirmation bearer access requires at least one successful reservation.
...
begin
  ...
  select ct.id, ct.registration_group_id
    into v_token_id, v_group_id
  ...
  if v_group_id is null or not exists (
    select 1
     ...
     where gr.registration_group_id = v_group_id
       and gr.success
  ) then
    raise exception 'invalid confirmation' using errcode = '42501';
  end if;
