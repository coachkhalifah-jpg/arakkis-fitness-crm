-- Phase 5C forward fix: cast trigger-generated transition sources to the enum.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    values (new.id, null, new.status, actor, (case when new.status = 'NO_SHOW' then 'FINALIZE' else 'CHECK_IN' end)::public.attendance_transition_source);
...
begin
  ...
  if char_length(btrim(coalesce(p_reason, ''))) = 0 and e.attendance_processing_state in ('REOPENED', 'FINALIZED') then raise exception 'correction reason is required' using errcode = '22023'; end if;
...
-- The walk-in RPC explicitly supplies a nullable id while reserving an id for
-- over-capacity overrides. Ensure ordinary registrations still receive their
-- normal UUID when that value is null.
create or replace function public.ensure_registration_id()
...
drop trigger if exists registration_id_default on public.registrations;
create trigger registration_id_default
before insert on public.registrations
for each row execute function public.ensure_registration_id();

-- Keep the local id used by the attendance write identical to the inserted row.
...
begin
  ...
  if walkin_registration_id is null then
    ...
    walkin_registration_id := gen_random_uuid();
