-- Pilot UX: optional post-registration communication link.
alter table public.events
  add column if not exists communication_url text,
  add column if not exists communication_label text;

alter table public.events
  add constraint events_communication_url_check
  check (communication_url is null or (char_length(communication_url) between 1 and 2048 and communication_url ~ '^https://')),
  add constraint events_communication_label_check
  check (communication_label is null or char_length(btrim(communication_label)) between 1 and 100);
