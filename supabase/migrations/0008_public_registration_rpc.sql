-- Phase 1 / 0008: narrow anonymous registration transaction.
-- This is the only anonymous write surface. It returns submission-scoped results and one raw,
-- immediately usable confirmation token; the token is never persisted in plaintext.
...
begin
  ...
  foreach event_id in array p_event_ids loop
    ...
    if event_row.visibility = 'AFFILIATION_RESTRICTED' then
      select exists (select 1 from public.event_eligible_organizations x where x.event_id = event_id and x.organization_id = p_primary_affiliation_organization_id) into affiliation_allowed;
