...
begin
  ...
  foreach v_id in array p_legal_document_version_ids loop
    if not exists (select 1 from public.acknowledgment_versions v where v.id=v_id and v.type in ('PARTICIPATION_RISK','LIABILITY_WAIVER','CANCELLATION_POLICY','TERMS_OF_USE','MEDIA_CONSENT') and v.legal_status='APPROVED' and v.effective_at<=now() and v.retired_at is null) then raise exception 'invalid legal acknowledgment version' using errcode='22023'; end if;
