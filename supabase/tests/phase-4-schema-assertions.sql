...
begin
  ...
  if has_function_privilege('anon', 'public.register_selected_events(text,text,text,text,text,text,text,uuid,text,text,uuid[],uuid,uuid,timestamptz,timestamptz,inet,text,text)', 'EXECUTE') then raise exception 'legacy anonymous registration execution remains'; end if;
  if not has_function_privilege('anon', 'public.register_selected_events_with_legal(text,text,text,text,text,text,text,text,uuid[],uuid,uuid,timestamptz,timestamptz,inet,text,text,text,text,uuid[])', 'EXECUTE') then raise exception 'legal registration execution is missing'; end if;
